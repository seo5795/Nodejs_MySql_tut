...
exports.search = function (request, response) {//검색 결과 출력페이지
  ...
  db.query(`SELECT * FROM topic LEFT JOIN author ON topic.author_id=author.id WHERE ${sql} LIKE ?`,  ["%" + queryData.search +"%"], function (error, topics) {
    ...
    while(i<topics.length){
      body+=`
      <h2>${sanitizeHtml(topics[i].title)}</h2>
      ${sanitizeHtml(topics[i].description)}`
      i+=1;
    }
    var html = template.HTML(title, list,
      body,
