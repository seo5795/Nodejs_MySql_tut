    ...
    db.query(`SELECT * FROM topic LEFT JOIN author ON topic.author_id=author.id WHERE topic.id=?`, [queryData.id], function (error2, topic) {
      ...
      var html = template.HTML(title, list,
        ...
        `
            <form action="search" method="get">
              <select>
                <option value="title" name="title">타이틀</option>
                <option value="author" name="author">저자</option>
              </select>
                <input type="text" name="search">
                <input type="submit" value="검색">
            </form>`,
...
exports.search = function (request, response) {//검색 결과 출력페이지
  ...
  if(queryData.category==='title'){//타이틀로 검색시
     sql='SELECT * FROM topic LEFT JOIN author ON topic.author_id=author.id WHERE topic.title LIKE ?';
     console.log(1);
  }else if(queryData.category==='author'){//저자로 검색시
     sql='SELECT * FROM topic LEFT JOIN author ON topic.author_id=author.id WHERE author.name LIKE ?';
    console.log(2);
  }
  db.query(sql,  ["%" + queryData.search +"%"], function (error, topics) {
